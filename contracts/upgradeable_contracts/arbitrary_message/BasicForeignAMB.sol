...
contract BasicForeignAMB is BasicAMB, MessageRelay, MessageDelivery {
    ...
    function _allowMessageExecution(bytes _data, bytes _signatures) internal {
        ...
        require(messageId() == bytes32(0), "messageId != 0");
    ...
    function _executeMessage(
        ...
        require(_isMessageVersionValid(msgId), "invalid message version");
        require(_isDestinationChainIdValid(chainIds[1]), "invalid destination chain id");
        require(!relayedMessages(msgId), "message has been relayed");
    ...
    function _validateExecutionStatus(bool _status) internal {
        require(_status || msg.sig == this.executeSignatures.selector, "invalid message execution status");
    }
