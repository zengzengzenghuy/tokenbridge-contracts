    ...
    BlockRewardBridge
{
    ...
    function nativeTransfer(address _receiver) internal {
        ...
        require(withinLimit(msg.value), "msg.value exceed limit");
        IBlockReward blockReward = blockRewardContract();
        uint256 totalMinted = blockReward.mintedTotallyByBridge(address(this));
        uint256 totalBurnt = totalBurntCoins();
        require(msg.value <= totalMinted.sub(totalBurnt), "msg.value must less than totalMinted.sub(totalBurnt)");
    ...
    function rewardableInitialize(
        ...
        require(AddressUtils.isContract(_feeManager), "fee manager is not a contract");
    ...
    function _initialize(
        ...
        require(AddressUtils.isContract(_validatorContract), "validatorContract is not a contract");
        require(
            _blockReward == address(0) || AddressUtils.isContract(_blockReward),
            "blockRFeward must be a contract and not 0"
        );
    ...
    function onExecuteAffirmation(
        ...
        IBlockReward blockReward = blockRewardContract();
        require(blockReward != address(0), "blockReward must not be 0");
    ...
    function onFailedAffirmation(address _recipient, uint256 _value, bytes32 _txHash, bytes32 _hashMsg) internal {
        ...
        require(recipient == address(0) && value == 0, "reciepint and value must not be 0");
