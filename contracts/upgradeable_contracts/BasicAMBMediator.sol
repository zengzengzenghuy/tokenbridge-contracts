...
contract BasicAMBMediator is Ownable {
    ...
    modifier onlyMediator() {
        require(msg.sender == address(bridgeContract()), "msg.sender must be bridge contract");
        require(
            messageSender() == mediatorContractOnOtherSide(),
            "original message sender must be mediator contract on other side"
        );
        _;
    }
    ...
    function _setBridgeContract(address _bridgeContract) internal {
        require(AddressUtils.isContract(_bridgeContract), "bridge contract must be a contract");
    ...
    function _setRequestGasLimit(uint256 _requestGasLimit) internal {
        require(_requestGasLimit <= maxGasPerTx(), "new request Gas limit must be maxGasPerTx");
