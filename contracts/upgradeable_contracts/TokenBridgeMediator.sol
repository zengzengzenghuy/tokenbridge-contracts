...
contract TokenBridgeMediator is BasicAMBMediator, BasicTokenBridge, TransferInfoStorage {
    ...
    function requestFailedMessageFix(bytes32 _messageId) external {
        require(!bridgeContract().messageCallStatus(_messageId), "bridge contract messageCallStatus fail");
        require(
            bridgeContract().failedMessageReceiver(_messageId) == address(this),
            "bridge contract failed message receiver fail"
        );
        require(
            bridgeContract().failedMessageSender(_messageId) == mediatorContractOnOtherSide(),
            "bridge contract failed message Sender is not mediatorContractOnOtherSide"
        );
    ...
    function fixFailedMessage(bytes32 _messageId) external onlyMediator {
        require(!messageFixed(_messageId), "messageFixed fail");
